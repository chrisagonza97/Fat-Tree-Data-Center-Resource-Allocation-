...
public class TestAlgo
{
    public static void main (String args[]){
        ...
        int k=1;
        ArrayList<Node> tree0= new ArrayList<Node>();
        ...
        for(y=0;y<(k*k*k)/4;y++){
                ...
                tree.get(id).setesc(set);//setting the initial effective storage capacity p
       ...
       ;
        ...
        for (int i =0;i<pmcount;i++){
        	if(tree.get(i+firstpm).getissource()==true){
        		lpfile+="Y"+i+"=1; \n";
        	}
        }
        ...
        for(int i=0;i<pmcount;i++){//loop for pm
        	if(tree.get(i+firstpm).getissource()==false){
        		for(int j=0;j<p;j++){
        			for(int l=0;l<r-1;l++){
        ...
        for(int i=0;i<p;i++){
        	for(int j=0;j<r-1;j++){
        		for(int l=0;l<pmcount;l++){
        			int origipm= vms.get(i).getpmid();//pm of original vm
        			int copycost=vms.get(i).getCCost(j);
        			int tempd=distance(tree,tree.get(origipm),tree.get(l+firstpm),k);
      ...
      //memory capacity constraint of all the PMs (ESC)
        for (int i=0;i<pmcount;i++){
        	int esc=tree.get(i+firstpm).getesc();
        	for (int h=0;h<esc;h++){
        	lpfile+="Y"+i;
        			if(h==esc-1){
        				lpfile+= ">= ";
      ...
      //cpms can be potentially turned off
            ...
            for (int j=0; j<pmcount;j++){//for each CPM with i copies
                ...
                for (int l=0;l<tree.get(j+firstpm).getvmclength();l++){//line 4
                    ...
                    for (int h =0; h<pmcount;h++){
                       ...
                       if(tree.get(h+firstpm).getissource()==false)
                        continue;
                       if (!(thiscost==temp)){
                           //aflag=false;
        ...
        for(int i =1; i<=m;i++){//line 1 of algorithm
            for (int j=0; j<pmcount;j++){//for each CPM with i copies
                boolean aflag = true;
                if (!(tree0.get(j+firstpm).getactive()&& tree0.get(j+firstpm).getissource()==false)){
                    
                	continue;//if this PM is not a CPM continue
                }
                if(tree0.get(j+firstpm).getreplicount()!=i){
                	
                    continue; //if this CPM does not have i replicas continue;
                }
                for (int l=0;l<tree0.get(j+firstpm).getvmclength();l++){//line 4
                    for (int h =0; h<pmcount;h++){
                       int thiscost= tree0.get(j+firstpm).getvmcost(l);//cost of replicating this
                       int temp=distance (tree0,tree0.get(h+firstpm),tree0.get(tree0.get(j+firstpm).getcpm(l)),k);
                       if (!(thiscost==temp)){
                           //aflag=false;
                    	   if(h==pmcount-1){
                    		   aflag=false;
                       ...
                       //System.out.println("0");
                       if(tree0.get(h+firstpm).getactive()==false){
                    	   if(h==pmcount-1){
                    		   aflag=false;
                    	   }
                           continue; //if PM is not active, it is not a TPM
                      ...
                      /* if(tree.get(h+firstpm).getissource()==true){
                       ...
                       //System.out.println("1");
                       if(tree0.get(h+firstpm).getvmcount()>=m){
                           //aflag=false;
                    	   if(h==pmcount-1){
                    		   aflag=false;
                    	   }
                       ...
                       //System.out.println("2");
                       //check if PM has another replica of this VM replica
                       if(tree0.get(h+firstpm).checkVm(tree0.get(j+firstpm).getvmid(l))){
                           //aflag=false;
                    	   if(h==pmcount-1){
                    		   aflag=false;
                    	   }
                       ...
                       //if loop hasn't continued, PM is a TPM
                       //move vm to PM, remove this vm copy
                       int movedvm = tree0.get(j+firstpm).getvmid(l);
                       aflag=true;
                       System.out.println("--VM "+tree0.get(j+firstpm).getvmid(l)+ " from PM " +(j+firstpm)+" migrated to PM with id " + (h+firstpm));
                       tree0.get(h+firstpm).addVMc(movedvm,h+firstpm,thiscost,m);
                       tree0.get(j+firstpm).removeVMc(tree0.get(j+firstpm).getvmid(l));
                       l--;
                       //System.out.println("VM "+tree.get(j+firstpm).getvmid(l)+ "from PM " +(j+firstpm)+"migrated to PM with id " + (h+firstpm));
      ...
      int actpm=0;
        for (int i=firstpm;i<=lastpm;i++){
            String print = tree0.get(i).printVms();
            if(tree0.get(i).getactive()){
            	actpm++;
            }
            if(print==null)
            continue;
        ...
        //System.out.println("Placement of VMS0 after consolidation:");
        /*for (int i=firstpm;i<=lastpm;i++){
            String print = tree0.get(i).printVms();
            if(print==null)
            continue;
       ...
       // System.out.println("number of IPMs: "+ IPM0);
        //System.out.println("number of IPMs from modified consolidation algorithm is "+IPM);
        //System.out.println("number of IPMs from original consolidation algorithm is "+ IPM0);
